...
# 1/ si la variable {{ cookiecutter.__command_slug.upper() }}_CONFIG est initialisée et que le fichier existe
# 2/ sinon on teste nomDuProjet.config dans le dossier courant. TODO: changer nomDuProjet
# 3/ si 1/ + 2/ échouent, on ne fait rien => ça chargera les variables {{ cookiecutter.__command_slug.upper() }}_XX
project_config() {
  # initialisation
  CONFIG_FILE=""
  local CONFIG_FILENAME="${PROJECT}.config"
  local CURRENTDIR_CONFIG="${PWD}/${CONFIG_FILENAME}"
  # 1/ si {{ cookiecutter.__command_slug.upper() }}_CONFIG existe, on l'utilise comme source de configuration.
  if [ -n "${{cookiecutter.__command_slug.upper()}}_CONFIG" ] && [ -f "${{cookiecutter.__command_slug.upper()}}_CONFIG" ]; then
    CONFIG_FILE="${{cookiecutter.__command_slug.upper()}}_CONFIG"
...
# On charge ensuite la configuration du projet (soit {{ cookiecutter.__command_slug.upper() }}_CONFIG, soit 
# nomDuProjet.config)
# Finalement on surcharge les variables avec celles données en ligne de
# commande (par {{ cookiecutter.__command_slug.upper() }}_variable=).
configure() {
